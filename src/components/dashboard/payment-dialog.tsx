...
export function PaymentDialog({ order, total: totalAmountDue, isOpen, onOpenChange, onConfirmPayment, clients }: PaymentDialogProps) {
  ...
  const clientBalance = useMemo(() => {
    ...
    // Calculate available balance to use for payment.
    // client.balance is the Net Balance (Credits - All Debts).
    // Since we are paying for *this* order, we need to consider the credit *backing* this order as available.
    // Available = NetBalance + CurrentOrderDebt.
    const orderTotal = order.items.reduce((acc, item) => acc + item.menuItem.price * item.quantity, 0);
    const orderPaid = order.payments?.reduce((acc, p) => acc + p.amount, 0) || 0;
    const currentOrderDebt = orderTotal - orderPaid;

    return (client.balance || 0) + currentOrderDebt;
  }, [order, clients]);
