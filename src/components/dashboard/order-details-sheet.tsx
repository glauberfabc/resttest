...
export function OrderDetailsSheet({ order, allOrders, allClients, menuItems, onOpenChange, onUpdateOrder, onProcessPayment, onDeleteOrder, printedKitchenItems, onSetPrintedItems }: OrderDetailsSheetProps) {
    ...
    const { previousDebt, dailyConsumption } = useMemo(() => {
        ...
        if (order.type === 'name') {
            ...
            if (client) {
                // client.balance is the Net Balance (Credits - Debts) including the current order
                // previousDebt is the balance BEFORE this order.
                // So we add back the *current order's remaining debt* to the balance.
                // currentOrderRemainingDebt = total - paidAmount.
                // If balance is -150 and current debt is 50, then previous was -100.
                // If balance is 100 and current debt is 50, then previous was 150.

                // However, we must ensure 'paidAmount' here is accurate to what the DB considers 'paid'.
                // paidAmount comes from order.payments reducer.

                const currentOrderDebt = total - paidAmount;
                debt = (client.balance || 0) + currentOrderDebt;
            }
        }
    ...
    }, [order, allClients, total, paidAmount]);
